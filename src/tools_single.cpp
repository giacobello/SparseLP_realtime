...
float tools_nrm2( int size, float *x){
 ...
 return snrm2_ ( &size, x, &one);
}

float tools_asum( int size, float *x){
 int one = 1;

 return sasum_ ( &size, x, &one);
}
...
float tools_inf( int size, float *x){
 int one = 1;
# 156 "tools.cpp"
 return fabs( x[ isamax_ ( &size, x, &one) - 1 ] );
}
...
# 173 "tools.cpp"
 for( i = 0 ; i < size_n ; ++i ){
  scopy_ ( &size_m, &X[i*size_m], &one, &Z[i*size_m], &one);
  sscal_ ( &size_m, &d[i], &Z[i*size_m], &one);
 }
...
# 193 "tools.cpp"
 for( j = 0 ; j < size_m ; ++j ){
  scopy_ ( &size_n, &X[j], &size_m, &Z[j], &size_m);
  sscal_ ( &size_n, &d[j], &Z[j], &size_m);
 }
...
void tools_ad( int size, float *d, float *X){
 ...
 int sizep1 = size + 1;
# 214 "tools.cpp"
 saxpy_ ( &size, &done, d, &one, X, &sizep1);
}
...
void tools_potrf( const char uplo, const int n, float* a,
