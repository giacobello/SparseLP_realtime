...
double tools_stepsize( int size, double *dx, double *x){
 ...
 for( i = 0 ; i < size ; ++i ){
  if( dx[i] < 0 )
   alpha = alpha < -x[i]/dx[i] ? alpha : -x[i]/dx[i];
 }
 return alpha;
}
...
double tools_stepsize_scale( int size, double scale, double *dx, double *x){
 ...
 for( i = 0 ; i < size ; ++i ){
  if( dx[i] < 0 )
   alpha = alpha < -scale*x[i]/dx[i] ? alpha : -scale*x[i]/dx[i];
 }
 return alpha;
}
...
double tools_nrm2( int size, double *x){
 ...
 return dnrm2_ ( &size, x, &one);
}

double tools_asum( int size, double *x){
 int one = 1;

 return dasum_ ( &size, x, &one);
}
...
double tools_inf( int size, double *x){
 int one = 1;
# 156 "tools.cpp"
 return fabs( x[ idamax_ ( &size, x, &one) - 1 ] );
}
...
# 173 "tools.cpp"
 for( i = 0 ; i < size_n ; ++i ){
  dcopy_ ( &size_m, &X[i*size_m], &one, &Z[i*size_m], &one);
  dscal_ ( &size_m, &d[i], &Z[i*size_m], &one);
 }
...
# 193 "tools.cpp"
 for( j = 0 ; j < size_m ; ++j ){
  dcopy_ ( &size_n, &X[j], &size_m, &Z[j], &size_m);
  dscal_ ( &size_n, &d[j], &Z[j], &size_m);
 }
...
void tools_ad( int size, double *d, double *X){
 ...
 int sizep1 = size + 1;
# 214 "tools.cpp"
 daxpy_ ( &size, &done, d, &one, X, &sizep1);
}
...
void tools_potrf( const char uplo, const int n, double* a,
