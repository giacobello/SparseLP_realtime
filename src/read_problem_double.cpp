...
# 5 "read_problem.cpp" 2
# 21 "read_problem.cpp"
# 1 "core_dual_double.h" 1
# 1 "core_dual.h"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "core_dual.h"
...
void init_problem( int m, int n, isignal_double *sig){
  sig->Y = (double*) malloc( m*n*sizeof(double));
  sig->y = (double*) malloc( m*sizeof(double));
}
...
int read_problem(char *s, isignal_double *sig){
 ...
 if( f != __null ){
  ...
  for( i = 0 ; i < sig->m*sig->n ; ++i ){
   if(fscanf(f,"%lf", &r) != 1) return 0; else sig->Y[i]=(double)r;
    }
  for( i = 0 ; i < sig->m ; ++i ){
   if(fscanf(f,"%lf", &r) != 1) return 0; else sig->y[i]=(double)r;
