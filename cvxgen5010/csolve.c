...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  const char *var_names[] = {"alpha", "s", "t"};
  ...
  if (xm == NULL) {
    ...
    if (!((mxGetM(xm) == 50) && (mxGetN(xm) == 10))) {
      printf("X must be size (50,10), not (%d,%d).\n", mxGetM(xm), mxGetN(xm));
      this_var_errors++;
    }
  ...
  if (xm == NULL) {
    ...
    if (!((mxGetM(xm) == 50) && (mxGetN(xm) == 1))) {
      printf("x must be size (50,1), not (%d,%d).\n", mxGetM(xm), mxGetN(xm));
      this_var_errors++;
    }
